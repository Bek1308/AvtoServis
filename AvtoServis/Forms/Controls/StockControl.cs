...
namespace AvtoServis.Forms.Controls
{
    public partial class StockControl : UserControl
    {
        ...
        {
            toolTip.SetToolTip(tableLayoutPanel, "Основная панель управления складами");
            toolTip.SetToolTip(titleLabel, "Заголовок раздела складов");
            ...
            toolTip.SetToolTip(addButton, "Добавить новый склад");
            ...
            toolTip.SetToolTip(countLabel, "Количество отображаемых складов");
            toolTip.SetToolTip(dataGridView, "Таблица с данными о складах");
        }
        ...
        private void BtnOpenFilterDialog_Click(object sender, EventArgs e)
        {
            ...
            {
                UpdateVisibleColumns();
                using (var dialog = new StockFilterDialog(_viewModel))
                {
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (var saveFileDialog = new SaveFileDialog())
                {
                    ...
                    saveFileDialog.FileName = $"Stocks_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        public void LoadData()
        {
            try
            {
                _dataSource = _viewModel.LoadStocks();
                SortDataSource();
                RefreshDataGridView();
        ...
        private void PerformSearch()
        {
            try
            {
                ...
                {
                    _dataSource = _viewModel.SearchStocks(searchBox.Text.Trim());
                }
                SortDataSource();
                RefreshDataGridView();
            }
        ...
        private void ViewModel_StocksChanged(object sender, EventArgs e)
        {
            LoadData(); // Hodisa chaqirilganda DataGridView ni yangilash
        }
        ...
        public void ShowDialog(int? id, bool isViewOnly = false)
        {
            try
            {
                using (var dialog = new StockDialog(_viewModel, id, isViewOnly))
                {
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        LoadData();
        ...
        private void RefreshDataGridView()
        {
            ...
            foreach (var stock in _dataSource)
            {
                ...
                if (_columnVisibility["StockID"])
                    row.Cells[dataGridView.Columns["StockID"]?.Index ?? 0].Value = stock.StockID;
                if (_columnVisibility["Name"])
                    row.Cells[dataGridView.Columns["Name"]?.Index ?? 0].Value = stock.Name;
