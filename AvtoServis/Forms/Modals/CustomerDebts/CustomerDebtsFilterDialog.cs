...
{
    public partial class CustomerDebtsFilterDialog : Form
    {
        ...
        public CustomerDebtsFilterDialog(CustomerDebtsViewModel viewModel)
        {
            InitializeComponent();
        ...
        {
            toolTip.SetToolTip(tableLayoutPanel, "Форма для фильтрации должников");
            toolTip.SetToolTip(titleLabel, "Заголовок фильтров должников");
        ...
        private void LoadExistingFilters()
        {
            ...
            foreach (var filter in Filters)
            {
                ...
                if (filter.Column == "RegistrationDate")
                {
                ...
                else if (filter.Column == "IsActive")
                {
                    ...
                    comboBoxStatus.Items.AddRange(new[] { "Да", "Нет" });
        ...
        private void UpdateOperatorComboBox(ComboBox cmbColumn, ComboBox cmbOperator, string savedOperator = null)
        {
            ...
            var selectedColumn = _columnMapping.FirstOrDefault(kvp => kvp.Value == cmbColumn.SelectedItem?.ToString()).Key;
            if (selectedColumn == "CustomerID" || selectedColumn == "UmumiyQarz")
            {
                cmbOperator.Items.AddRange(new[] { "=", ">", "<", ">=", "<=", "!=" });
            }
            else if (selectedColumn == "RegistrationDate")
            {
                cmbOperator.Items.AddRange(new[] { "=", ">", "<", ">=", "<=" });
            }
            else if (selectedColumn == "IsActive")
            {
                cmbOperator.Items.AddRange(new[] { "=", "!=" });
            }
            else
            {
                cmbOperator.Items.AddRange(new[] { "LIKE", "=", "!=" });
            }
        ...
        private void UpdateInputControlAndOperator(ComboBox cmbColumn, FlowLayoutPanel panel, ComboBox cmbOperator, Button btnRemove)
        {
            ...
            if (selectedColumn == "RegistrationDate")
            {
            ...
            else if (selectedColumn == "IsActive")
            {
                ...
                ((ComboBox)newInputControl).Items.AddRange(new[] { "Да", "Нет" });
        ...
        private void BtnApply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _viewModel.LoadCustomerDebtsAsync();
