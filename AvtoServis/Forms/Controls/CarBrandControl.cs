...
namespace AvtoServis.Forms.Controls
{
    public partial class CarBrandControl : UserControl
    {
        ...
        {
            toolTip.SetToolTip(tableLayoutPanel, "Основная панель управления марками автомобилей");
            toolTip.SetToolTip(titleLabel, "Заголовок раздела марок автомобилей");
            ...
            toolTip.SetToolTip(addButton, "Добавить новую марку автомобиля");
            ...
            toolTip.SetToolTip(countLabel, "Количество отображаемых марок");
            toolTip.SetToolTip(dataGridView, "Таблица с данными о марках автомобилей");
        }
        ...
        private void BtnOpenFilterDialog_Click(object sender, EventArgs e)
        {
            ...
            {
                UpdateVisibleColumns();
                using (var dialog = new CarBrandFilterDialog(_viewModel))
                {
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (var saveFileDialog = new SaveFileDialog())
                {
                    ...
                    saveFileDialog.FileName = $"CarBrands_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        private void DataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (dataGridView.Columns[e.ColumnIndex].Name == "Actions")
                {
                    ...
                    {
                        using (var dialog = new CarBrandDialog(_viewModel, id, isDeleteMode: true))
                        {
                            if (dialog.ShowDialog() == DialogResult.OK)
                            {
                                LoadData();
        ...
        public void LoadData()
        {
            try
            {
                _dataSource = _viewModel.LoadBrands();
                SortDataSource();
                RefreshDataGridView();
        ...
        private void PerformSearch()
        {
            try
            {
                ...
                {
                    _dataSource = _viewModel.SearchBrands(searchBox.Text.Trim());
                }
                SortDataSource();
                RefreshDataGridView();
            }
        ...
        public void ShowDialog(int? id, bool isViewOnly = false)
        {
            try
            {
                using (var dialog = new CarBrandDialog(_viewModel, id, isViewOnly))
                {
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        LoadData();
        ...
        private void RefreshDataGridView()
        {
            ...
            foreach (var brand in _dataSource)
            {
                var row = new DataGridViewRow();
                row.CreateCells(dataGridView);
                if (_columnVisibility["Id"])
                    row.Cells[dataGridView.Columns["Id"]?.Index ?? 0].Value = brand.Id;
                if (_columnVisibility["CarBrandName"])
                    row.Cells[dataGridView.Columns["CarBrandName"]?.Index ?? 0].Value = brand.CarBrandName;
            ...
            countLabel.Text = $"Марки: {_dataSource.Count}";
